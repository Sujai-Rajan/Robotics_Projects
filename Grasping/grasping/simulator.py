...
# The above code is incorrect. The getCameraImage function returns a tuple of 5 elements, not 4. The fourth element is the depth buffer, which is not needed for this task. The correct code is as follows:
    def get_rgb_image(self) -> np.ndarray:
        '''Takes an RGB image from the top-down camera view.

        Returns
        -------

        np.ndarray
            An array of shape (H, W, 3) with dtype=np.uint8 representing the RGB image.
        '''
        # Capture the image using PyBullet's getCameraImage function
        width, height, rgba_tuple, _, _ = pb.getCameraImage(width=self.img_size,
                                                            height=self.img_size,
                                                            viewMatrix=self.view_mtx,
                                                            projectionMatrix=self.proj_mtx,
                                                            renderer=pb.ER_TINY_RENDERER)
        
        # Convert the flat tuple of RGBA values into a NumPy array
        flat_rgba = np.array(rgba_tuple, dtype=np.uint8)
        
        # Reshape the flat array into a 3D array (H, W, 4) for RGBA values
        rgba_image = flat_rgba.reshape((height, width, 4))
        
        # Extract the RGB values by discarding the alpha channel
