...
class MobileUNet(nn.Module):
    ...
    def forward(self, img: Tensor) -> Tensor:
        ...
        # Upsample and process the feature maps from both branches
        x_main = self.branch_up(x_narrow)  # Now (B, 4, 32, 32)
        x_sub = self.branch_down(x_wide)  # Now (B, 4, 32, 32)
        
        # Combine the feature maps from the two branches
        combined = x_main + x_sub  # Shape: (B, 4, 32, 32)

        # Apply 1x1 conv to get the final logits
...
def main():
    ...
    model = MobileUNet(pretrained=True).to(DEVICE) # Sent to device- CPU or GPU
    ...
    for epoch_num in pbar:
        ...
        print(f'Epoch {epoch_num} | Train Loss: {avg_train_loss:.2f} | Val Loss: {avg_val_loss:.2f}')
